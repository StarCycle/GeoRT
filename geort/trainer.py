...
class GeoRTTrainer:
    ...
    def get_robot_neural_fk_model(self, force_train=False):
        ...
        fk_model = FKModel(keypoint_joints=self.get_keypoint_info()["joint"])
    ...
    def train(self, human_data_path, **kwargs):
        ...
        ik_model = IKModel(keypoint_joints=self.get_keypoint_info()["joint"])
        os.makedirs("./checkpoint", exist_ok=True)
        ...
        for epoch in range(n_epoch):
            for batch_idx, batch in enumerate(point_dataloader):
                ...
                target = torch.from_numpy(robot_points[:, selected_idx, :]).permute(1, 0, 2).float()
                ...
                # [Direction Loss]
                direction = F.normalize(torch.randn_like(point), dim=-1, p=2)
                scale = 0.001 + torch.rand(point.size(0)).unsqueeze(-1).unsqueeze(-1) * 0.01
