...
class HandFormatter:
    ...
    def normalize_torch(self, x):
        return ((x - torch.from_numpy(self.joint_lower_limit).to(x.device)) / (torch.from_numpy(self.joint_upper_limit).to(x.device) - torch.from_numpy(self.joint_lower_limit).to(x.device)) - 0.5) * 2
